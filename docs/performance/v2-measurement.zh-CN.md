...
优化后的网络运行仍发生上传交付失败。
后续[接收额度对照](v2-credit-20260905.zh-CN.md)修复了确定性预留停滞，网络失败仍未解决。
[接收状态诊断](v2-receive-20260905.zh-CN.md)补充实时额度/组状态与剩余上传失败的新 profile。
[协商路径对照](v2-paths-20260905.zh-CN.md)消除了观察到的短时上传停滞，并公开对应
profile 和本地基准；正式吞吐与容量验收仍未完成。
[分配成本对照](v2-allocations-20260905.zh-CN.md)记录 v2 发送端分配降低、
有升有降的吞吐变化及独立传输计时，不证明吞吐改善的因果关系。
[方向认证状态对照](v2-authenticator-20260905.zh-CN.md)记录预付 HMAC 状态后的
v2 发送/接收分配成本降低，同时保留吞吐有升有降及主机压力限制。
[预付发送工作区对照](v2-send-workspace-20260905.zh-CN.md)保障满字节额度下已接纳
原始报文的推进，并记录同负载的分配、吞吐及主机压力证据。
